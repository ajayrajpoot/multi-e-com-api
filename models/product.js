...
const getProduct = async (filter) => {
    ...
    if (Number(filter.textSearch)) {
        condition += ` and title = '%${filter.textSearch}%' OR discription = '%${filter.textSearch}%'`;
        // val.push(filter.category_id);
    }
    ...
    const [result] = await readDB.query(`SELECT * FROM product ${condition}`, val);
    return result;
};
